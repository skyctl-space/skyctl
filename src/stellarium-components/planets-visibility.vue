...
<template>
<v-dialog max-width='600' v-model="showPlanetsVisibilityDialog">
  <v-card v-if="showPlanetsVisibilityDialog" transparent class="secondary white--text">
    ...
    <v-card-text>
      <div>
        ...
        <template v-for="obj in objs" :key="obj.v">
        <v-row no-gutters >
    ...
    <v-card-actions>
      <v-spacer></v-spacer><v-btn class="blue--text darken-1" text @click.native="showPlanetsVisibilityDialog = false">Close</v-btn>
    </v-card-actions>
  </v-card>
</v-dialog>
</template>
...
const state = reactive({
  objs: [
    store.state.stel.getObj('NAME Sun'),
    store.state.stel.getObj('NAME Moon'),
    store.state.stel.getObj('NAME Mercury'),
    store.state.stel.getObj('NAME Venus'),
    store.state.stel.getObj('NAME Mars'),
    store.state.stel.getObj('NAME Jupiter'),
    store.state.stel.getObj('NAME Saturn')
  ]
})

const showPlanetsVisibilityDialog = computed(() => store.state.showPlanetsVisibilityDialog)

const formatTime = (jdm) => {
  const d = new Date()
  d.setMJD(jdm)
...
const sunBackgroundStr = computed(() => {
  const sun = store.state.stel.getObj('NAME Sun')
  const brightness = []
  const d = new Moment(startDate.value)

  const obs = store.state.stel.core.observer.clone()
  for (let i = 0; i < 25; i++) {
    obs.utc = d.toDate().getMJD()
    d.local()
    const azalt = store.state.stel.convertFrame(obs, 'ICRF', 'OBSERVED', sun.getInfo('radec', obs))
    const alt = store.state.stel.anpm(store.state.stel.c2s(azalt)[1]) * 180.0 / Math.PI
...
const startDate = computed(() => {
  const sun = store.state.stel.getObj('NAME Sun')
  let u = store.state.stel.observer.utc
  if (u < sun.rise) {
    u = u - 1
  }
  let d = new Date()
