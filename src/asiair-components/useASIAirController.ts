...
const asiairControllers = new Map<
    string,
    ReturnType<typeof createASIAirState>
>();
...
listen<string>("asiair_camera_state_change", async (event) => {
    ...
    if (controller) {
        ...
        await controller.update_avaliable_cameras();
...
listen<number>("asiair_main_camera_temperature", async (event: any) => {
    const guid = event.payload[0];
    const temperature = event.payload[1];
    const controller = asiairControllers.get(guid);
    if (controller) {
        console.log(`[${guid}] Camera temperature: ${temperature}`);
        controller.mainCamera.value.temperature = temperature;
    }
});
...
listen<ExposureEvent>("asiair_exposure", async (event: any) => {
    const guid = event.payload[0];
    const exposure: ExposureEvent = event.payload[1];

    console.log("Event: ", event);

    const controller = asiairControllers.get(guid);
    if (controller) {
        console.log(`[${guid}] Camera exposure event: ` + exposure.state);

        if (exposure.state === "complete") {
            await controller.main_camera_get_current_img();
        }
    }
});
...
async function updateCameraInfo(guid: string, mainCamera: Ref<Camera>) {
    ...
    await invoke<number>("main_camera_get_exposure", {
        guid: guid,
    })
        .then((exposure) => {
            mainCamera.value.exposure_us = exposure;
        })
        .catch((error: any) => {
            console.error(`[${guid}] Error getting camera exposure:`, error);
        });
}
...
function createASIAirState(guid: string, connection: string | undefined) {
    ...
    async function onConnection() {
        // Update list of cameras available
        await update_avaliable_cameras();

        // Update main camera state
        await main_camera_update_state();
    ...
    async function update_avaliable_cameras() {
        await invoke<ConnectedCamera[]>("get_connected_cameras", {
            ...
            .catch((error: any) => {
                console.error(
                    `[${state.guid}] Error getting connected cameras:`,
                    error
                );
    ...
    async function main_camera_update_state() {
        await invoke<CameraState>("main_camera_get_state", {
            guid: state.guid,
        })
            .then((cameraState) => {
                ...
                if (cameraState.state === "idle") {
                    ...
                    updateCameraInfo(guid, toRef(state.public, "mainCamera"));
                } else {
    ...
    async function trigger_capture() {
        await invoke("main_camera_start_exposure", {
            guid: state.guid,
        })
            .then(() => { })
            .catch((error: any) => {
                console.error(`[${state.guid}] Error starting camera exposure:`, error);
            });
    }

    async function main_camera_get_current_img() {
        let mainCamera = state.public.mainCamera;

        let imgNotificationChannel: Channel<ImageProgress> =
            new Channel<ImageProgress>();
    ...
    async function main_camera_open(camera_id: number) {
        var camera_name : any = null;
        for (const camera of state.public.availableCameras) {
    ...
    async function main_camera_set_exposure(exposure: number) {
        await invoke("main_camera_set_exposure", {
            ...
            .then(() => {
                state.public.mainCamera.exposure_us = exposure;
    ...
    return {
        ...
        main_camera_open,
        main_camera_close,
        main_camera_update_state,
        update_avaliable_cameras,
        main_camera_set_exposure,
        main_camera_get_current_img,
    };
}

export function useASIAirController(
    guid: string,
