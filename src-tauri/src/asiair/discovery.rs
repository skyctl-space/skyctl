...
use crate::{ASIAirState, DiscoveredASIAir};
...
pub fn start_asiair_discovery(app: AppHandle) {
    ...
    let devices_cache = app.state::<ASIAirState>().discovered.clone();

    log::info!("Starting Asiair discovery...");
    let _ = thread::Builder::new()
        .name("asiair_discovery".to_string())
        .spawn(move || {
            loop {
                if DISCOVERY_RUNNING.load(Ordering::SeqCst) {
                            ...
                            if let Some(result) = json_response.get("result") {
                                if let Some(guid) = result.get("guid").and_then(|v| v.as_str()) {
                                    if let Some(ip) = result.get("ip").and_then(|v| v.as_str()) {
                                        if let Some(ssid) =
                                            result.get("ssid").and_then(|v| v.as_str())
                                        {
                                            let device = DiscoveredASIAir {
                    ...
                    let consolidated_list: Vec<DiscoveredASIAir> =
                        cache.values().cloned().collect();
                    app_handle
                        .emit("discovered_device", consolidated_list)
                        .expect("Failed to emit device list");
                    thread::sleep(Duration::from_millis(2000)); // Wait before sending the next discovery message
                } else {
